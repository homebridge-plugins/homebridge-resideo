...
export class Thermostats extends deviceBase {
  ...
  constructor(
    ...
    if (device.thermostat?.hide_fan) {
      // Look the service up on the accessory. This used to test `this.Fan`,
      ...
      const existingFanService = accessory.getService(this.hap.Service.Fanv2)
      if (existingFanService) {
        this.debugLog(`${this.device.deviceType}: ${accessory.displayName} Removing Fan Service`)
        accessory.removeService(existingFanService)
      } else {
